...
WORKDIR /opt/android
RUN apt update && apt full-upgrade -y && apt install -y openjdk-17-jdk curl tar zip unzip
...
RUN mv cmdline-tools latest && mkdir cmdline-tools && mv latest cmdline-tools/
RUN yes | /opt/android/cmdline-tools/latest/bin/sdkmanager --install 'build-tools;34.0.0' 'platforms;android-34' 'emulator' 'platform-tools'
WORKDIR /opt
